...
func TestLogUnexpectedErrorsOnly(t *testing.T) {
	...
	for i, tc := range tests {
		...
		require.Error(t, (&MultiTenancyLayer{minio.GatewayUnsupported{}, nil, nil, nil, uplink.Config{}, false}).log(ctx, tc.input))
...
func TestLogAllErrors(t *testing.T) {
	...
	for i, tc := range tests {
		log := gwlog.New()
		ctx := log.WithContext(context.Background())
		require.Error(t, (&MultiTenancyLayer{minio.GatewayUnsupported{}, nil, nil, nil, uplink.Config{}, true}).log(ctx, tc.input))
		require.Equal(t, tc.expected, log.TagValue("error"), i)
	}
}

func TestInvalidAccessGrant(t *testing.T) {
	layer := &MultiTenancyLayer{minio.GatewayUnsupported{}, nil, nil, nil, uplink.Config{}, true}
