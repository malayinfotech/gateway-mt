...
import (
	...
	"gateway-mt/pkg/httpserver"
	"gateway-mt/pkg/minio"
	"gateway-mt/pkg/server/gw"
...
func New(config Config, log *zap.Logger, trustedIPs trustedip.List, corsAllowedOrigins []string,
	...
	layer, err := gw.NewMultiTenantLayer(miniogw.NewStorxGateway(config.S3Compatibility), satelliteConnectionPool, connectionPool, uplinkConfig, config.InsecureLogAll)
	if err != nil {
		return nil, err
	}
