...
func TestAddRequestID(t *testing.T) {
	...
	require.NotEqual(t, "", rw.Header().Get(XStorxRequestID), "RequestId is not set in response header")
}
...
func TestAddRequestIDHeader(t *testing.T) {
	...
	require.Equal(t, requestID, request.Header.Get(XStorxRequestID), "RequestID value is not set")
}
