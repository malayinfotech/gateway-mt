...
import (
	...
	"gateway-mt/pkg/auth/badgerauth/pb"
...
var minimalAccessSatelliteID = func() storx.NodeURL {
	url, err := storx.ParseNodeURL(minimalAccessSatelliteURL)
	if err != nil {
		panic(err)
	}
	return url
...
func TestResources_CRUD(t *testing.T) {
	...
	t.Run("Availability after startup", func(t *testing.T) {
		allowed := map[storx.NodeURL]struct{}{minimalAccessSatelliteID: {}}
	...
	t.Run("ApprovedSatelliteID", func(t *testing.T) {
		var unknownSatelliteID storx.NodeURL
		unknownSatelliteID.ID[4] = 7
		allowed := map[storx.NodeURL]struct{}{unknownSatelliteID: {}}
		res := newResource(t, logger, authdb.NewDatabase(kv, allowed), endpoint)
		...
		allowed = map[storx.NodeURL]struct{}{unknownSatelliteID: {}, minimalAccessSatelliteID: {}}
		res = newResource(t, logger, authdb.NewDatabase(kv, allowed), endpoint)
