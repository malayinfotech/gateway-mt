...
// the same format as https://www.storx.io/dcs-satellites.  HasNodeList indicates
// if any configValue is a node address list, indicating it should be polled for updates.
func LoadSatelliteURLs(ctx context.Context, configValues []string) (satMap map[storx.NodeURL]struct{}, hasNodeList bool, err error) {
	defer mon.Task()(&ctx)(&err)

	satMap = make(map[storx.NodeURL]struct{})
...
func readSatelliteList(input []byte, satellites map[storx.NodeURL]struct{}) (err error) {
...
func ParseSatelliteURL(s string) (id storx.NodeURL, err error) {
	url, err := storx.ParseNodeURL(s)
	if err != nil {
		return storx.NodeURL{}, ErrAllowedSatelliteList.Wrap(err)
	}

	if url.ID.IsZero() {
		nodeID, found := rpc.KnownNodeID(url.Address)
		if !found {
			return storx.NodeURL{}, ErrAllowedSatelliteList.New("unknown satellite %q", s)
		}
		url.ID = nodeID
	}
