...
func TestLoadSatelliteAddresses(t *testing.T) {
	...
	expected := make(map[storx.NodeURL]struct{})
	for _, a := range withIds {
		url, err := storx.ParseNodeURL(a)
...
func TestReadSatelliteList(t *testing.T) {
	validNodeURLs, err := storx.ParseNodeURLs("118UWpMCHzs6CvSgWd9BfFVjw5K9pZ" +
		"bJjkfZJexMtSkmKxvvAW@satellite.stefan-benten.de:7777,12EayRS2V1kEsWE" +
		"SU9QMRseFhdxYxKicsiFmxrsLZHeLUtdps3S@us1.storx.io:7777,12L9ZFwhzVpuE" +
		"KMUNUqkaTLGzwY9G24tbiigLiXpmZWKwmcNDDs@eu1.storx.io:7777,121RTSDpyNZ" +
		"VcEU84Ticf2L1ntiuUimbWgfATz21tuvgk3vzoA6@ap1.storx.io:7777,1wFTAgs9D" +
		...
		"mo@us2.storx.io:7777")
	require.NoError(t, err)

	tests := []struct {
		name               string
		input              []byte
		satellites         map[storx.NodeURL]struct{}
		expectedSatellites map[storx.NodeURL]struct{}
	}{
		{
			name:               "empty",
			satellites:         make(map[storx.NodeURL]struct{}),
			expectedSatellites: make(map[storx.NodeURL]struct{}),
		},
		{
			name: "comments only",
			input: []byte("# 118UWpMCHzs6CvSgWd9BfFVjw5K9pZbJjkfZJexMtSkmKxvv" +
				"AW@satellite.stefan-benten.de:7777\n#12L9ZFwhzVpuEKMUNUqkaTL" +
				"GzwY9G24tbiigLiXpmZWKwmcNDDs@eu1.storx.io:7777"),
			satellites:         make(map[storx.NodeURL]struct{}),
			expectedSatellites: make(map[storx.NodeURL]struct{}),
		},
		{
		...
		},
		{
			...
			input: []byte("12EayRS2V1kEsWESU9QMRseFhdxYxKicsiFmxrsLZHeLUtdps3" +
				...
				"pmZWKwmcNDDs@eu1.storx.io:7777\n121RTSDpyNZVcEU84Ticf2L1ntiu" +
				"UimbWgfATz21tuvgk3vzoA6@ap1.storx.io:7777\n1wFTAgs9DP5RSnCqK" +
				...
				"9t6aToeiwbo5mfS2rUmo@us2.storx.io:7777"),
			satellites: make(map[storx.NodeURL]struct{}),
			expectedSatellites: map[storx.NodeURL]struct{}{
				validNodeURLs[1]: {},
				validNodeURLs[2]: {},
		...
		},
		{
			...
			input: []byte("# https://www.storx.io/dcs-satellites\n\n\t\t\t\n " +
				...
				"tRQrMTWUWwzwGh18i7Fqs67kmdhH9t6aToeiwbo5mfS2rUmo@us2.storx.i" +
				"o:7777                                                      "),
			satellites: make(map[storx.NodeURL]struct{}),
			expectedSatellites: map[storx.NodeURL]struct{}{
				validNodeURLs[3]: {},
...
func TestReadSatelliteList_Invalid(t *testing.T) {
	for i, input := range []string{
		"121RTSDpyNZVcEU84Ticf2L1ntiuUimbWgfATz21tuvgk3vzoA6@ap1.storx.io:777" +
		...
		require.Error(t, readSatelliteList([]byte(input), make(map[storx.NodeURL]struct{})), i)
	}
}
...
func TestParseSatelliteURL(t *testing.T) {
	for i, tc := range []struct {
		...
		expectedID, err := storx.NodeIDFromString(tc.expectedID)
		require.NoError(t, err, i)
