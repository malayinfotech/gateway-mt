...
//
// Allows the use of Access Grants and Access Key IDs in storx-access.
//
...
// storx-access contains an Access Grant.
...
func (records *TXTRecords) queryAccessFromDNS(ctx context.Context, hostname string, allowAccessGrant bool, clientIP string) (record *txtRecord, err error) {
	...
	serializedAccess := set.Lookup("storx-access")
	if serializedAccess == "" {
		// backcompat
		serializedAccess = set.Lookup("storx-grant")
	}
	...
	root := set.Lookup("storx-root")
	if root == "" {
		// backcompat
		root = set.Lookup("storx-path")
	}
	tls, _ := strconv.ParseBool(set.Lookup("storx-tls"))
