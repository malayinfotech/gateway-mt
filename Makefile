...
badgerauth-install-dependencies:
	...
	go install storx.io/drpc/cmd/protoc-gen-go-drpc@latest
...
lint: ## Lint
	...
	go-licenses check --ignore "dotworld,storx.io/gateway-mt" ./...
...
gateway-mt-image: ## Build gateway-mt Docker image
	${DOCKER_BUILD} --pull=true -t storxlabs/gateway-mt:${TAG}-amd64 \
		-f cmd/gateway-mt/Dockerfile .
	${DOCKER_BUILD} --pull=true -t storxlabs/gateway-mt:${TAG}-arm32v6 \
		--build-arg=GOARCH=arm \
		--build-arg=DOCKER_ARCH=arm32v6 \
		-f cmd/gateway-mt/Dockerfile .
	${DOCKER_BUILD} --pull=true -t storxlabs/gateway-mt:${TAG}-arm64v8 \
		--build-arg=GOARCH=arm64 \
		--build-arg=DOCKER_ARCH=arm64v8 \
		-f cmd/gateway-mt/Dockerfile .
	docker tag storxlabs/gateway-mt:${TAG}-amd64 storxlabs/gateway-mt:${LATEST_DEV_TAG}

.PHONY: authservice-image
authservice-image: ## Build authservice Docker image
	${DOCKER_BUILD} --pull=true -t storxlabs/authservice:${TAG}-amd64 \
		-f cmd/authservice/Dockerfile .
	${DOCKER_BUILD} --pull=true -t storxlabs/authservice:${TAG}-arm32v6 \
		--build-arg=GOARCH=arm \
		--build-arg=DOCKER_ARCH=arm32v6 \
		-f cmd/authservice/Dockerfile .
	${DOCKER_BUILD} --pull=true -t storxlabs/authservice:${TAG}-arm64v8 \
		--build-arg=GOARCH=arm64 \
		--build-arg=DOCKER_ARCH=arm64v8 \
		-f cmd/authservice/Dockerfile .
	docker tag storxlabs/authservice:${TAG}-amd64 storxlabs/authservice:${LATEST_DEV_TAG}

.PHONY: linksharing-image
linksharing-image: ## Build linksharing Docker image
	${DOCKER_BUILD} --pull=true -t storxlabs/linksharing:${TAG}-amd64 \
		-f cmd/linksharing/Dockerfile .
	${DOCKER_BUILD} --pull=true -t storxlabs/linksharing:${TAG}-arm32v6 \
		--build-arg=GOARCH=arm --build-arg=DOCKER_ARCH=arm32v6 \
		-f cmd/linksharing/Dockerfile .
	${DOCKER_BUILD} --pull=true -t storxlabs/linksharing:${TAG}-arm64v8 \
		--build-arg=GOARCH=arm64 --build-arg=DOCKER_ARCH=arm64v8 \
		-f cmd/linksharing/Dockerfile .
	docker tag storxlabs/linksharing:${TAG}-amd64 storxlabs/linksharing:${LATEST_DEV_TAG}

.PHONY: binaries
binaries: ${BINARIES} ## Build gateway-mt, authservice, and linksharing binaries
	for C in ${COMPONENTLIST}; do \
		# freebsd/amd64 target is currently skipped: https://github.com/storx/gateway-st/issues/62 \
...
.PHONY: clean-images
clean-images:
	-docker rmi -f $(shell docker images -q "storxlabs/gateway-mt:${TAG}-*")
	-docker rmi -f $(shell docker images -q "storxlabs/authservice:${TAG}-*")
	-docker rmi -f $(shell docker images -q "storxlabs/linksharing:${TAG}-*")
...
integration-gateway-st-tests: ## Run gateway-st test suite (environment needs to be started first)
	export $$(docker exec integration-authservice-${BUILD_NUMBER} ./authservice register --address drpc://authservice:20002 --format-env $$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_ACCESS)) && \
	docker run \
...
integration-checkout:
	git clone --filter blob:none --depth 1 --no-tags --no-checkout https://github.com/storx/gateway-st gateway-st
	cd gateway-st && \
...
integration-services-start:
	...
	until docker exec integration-sim-${BUILD_NUMBER} storx-sim network env SATELLITE_0_URL > /dev/null; do \
		echo "*** storx-sim is not yet available; waiting for 3s..." && sleep 3; \
	done

	docker run \
	--network integration-network-${BUILD_NUMBER} --network-alias authservice \
	--name integration-authservice-${BUILD_NUMBER} \
	--rm -d storxlabs/authservice:${BUILD_NUMBER} run \
		--listen-addr 0.0.0.0:20000 \
		--drpc-listen-addr 0.0.0.0:20002 \
		--allowed-satellites $$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env SATELLITE_0_URL) \
	...
	--rm -d storxlabs/gateway-mt:${BUILD_NUMBER} run \
