...
import (
	...
	"common/storx"
	"common/testcontext"
	client "gateway-mt/internal/authadminclient"
	"gateway-mt/pkg/auth/authdb"
	"gateway-mt/pkg/auth/badgerauth"
	"gateway-mt/pkg/auth/badgerauth/badgerauthtest"
)
...
func TestGetRecord(t *testing.T) {
	badgerauthtest.RunCluster(t, badgerauthtest.ClusterConfig{
		...
		require.NoError(t, err)
		sk := encKey.ToStorxKey()
		encAccessGrant, err := encryption.Encrypt([]byte(testAccessGrant), storx.EncAESGCM, &sk, &storx.Nonce{1})
		require.NoError(t, err)
